...
double C(double x, double y) { return sin(a * x) * sin(a * y); }

double source(double x, double y) {
  return -a * a *
         (2. * dxy * cos(a * x) * cos(a * y) -
          (dx + dy) * sin(a * x) * sin(a * y));
  return (dx + dy) * a * a * sin(a * x) * sin(a * y);
}
...
void Problem::initProblem() {
  ...
  numDirNodes = 0;
  int glob_ind = 0;
  for (Mesh::iteratorNode inode = m.BeginNode(); inode != m.EndNode();
       inode++) {
...
void coords_from_barycentric(double *node_x, double *node_y, double *eta,
                             double *x, double *y) {
...
// [   ]
// [   ]
// energetic scalar product: [u,v] = (D*grad(u), grad(v)) = (grad(v))^T * D *
// grad(u)
void Problem::assembleLocalSystem(const Cell &c, rMatrix &A_loc,
                                  rMatrix &rhs_loc) {
  rMatrix D(2, 2);
  ...
  for (unsigned i = 0; i < 3; i++) {
    ...
    rMatrix u = basis_func_grad(c, nodes[i]);
    double u_0 = u(0, 0);

    // calc D * grad(u)
    u(0, 0) = D(0, 0) * u(0, 0) + D(0, 1) * u(1, 0);
    u(1, 0) = D(1, 0) * u_0 + D(1, 1) * u(1, 0);

    for (unsigned j = 0; j < 3; j++) {
      rMatrix v = basis_func_grad(c, nodes[j]);
      A_loc(i, j) = (v(0, 0) * u(0, 0) + v(1, 0) * u(1, 0));
    }
    rhs_loc(i, 0) = source(center_x, center_y) *
...
void Problem::assembleGlobalSystem(Sparse::Matrix &A, Sparse::Vector &rhs) {
  ...
  for (Mesh::iteratorCell icell = m.BeginCell(); icell != m.EndCell();
    ...
    for (unsigned loc_ind = 0; loc_ind < 3; loc_ind++) {
      ...
      if (nodes[loc_ind].GetMarker(mrkDirNode)) continue;
...
double Problem::get_c_norm() {
  ...
  for (Mesh::iteratorNode inode = m.BeginNode(); inode != m.EndNode();
       inode++) {
    Node n = inode->getAsNode();
    normC = max(fabs(n.Real(tagConc) - C(n.Coords()[0], n.Coords()[1])), normC);
  }
  return normC;
}
